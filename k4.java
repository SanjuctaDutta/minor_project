...
class Movie {
    ...
    public boolean matchesCriteria(String selectedGenre, String selectedDirectors, String selectedCast, String selectedCountry, String selectedLanguage,String selectedChoreographer) {
        return (selectedGenre.isEmpty() || genre.contains(selectedGenre)) &&
               ...
               (selectedChoreographer.isEmpty() || choreographer.contains(selectedChoreographer))&&
        ...
        public void handle(HttpExchange exchange) throws IOException {
            if ("POST".equals(exchange.getRequestMethod())) {
                try (InputStreamReader isr = new InputStreamReader(exchange.getRequestBody());
                    ...
                    String selectedDirectors=data.get("directors");
                    String selectedCast =data.get("cast");
                        ...
                        for (int relaxedCount = 1; relaxedCount <= 6; relaxedCount++) {
                            for (int combination = 0; combination <= 32; combination++) {
                                if (Integer.bitCount(combination) == relaxedCount) {
                                    ...
                                    selectedChoreographer = (combination & 32) == 32 ? "" : selectedChoreographer;
